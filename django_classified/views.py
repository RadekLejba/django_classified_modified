...
from .forms import ContactForm, ItemForm, ProfileForm, SearchForm
...
class ItemCreateView(FormsetMixin, CreateView):
    ...
    def dispatch(self, *args, **kwargs):
        ...
        if not profile.allow_add_item():
            ...
            return redirect(reverse('user-items'))
...
class ItemDeleteView(DeleteView):
    model = Item
    success_url = reverse_lazy('user-items')
...
class SetAreaView(View):
    def get(self, request):
        ...
        next_url = self.request.GET.get('next') or reverse_lazy('index')
